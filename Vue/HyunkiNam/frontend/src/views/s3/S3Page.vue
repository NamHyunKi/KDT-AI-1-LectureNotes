<template>
    <v-container>
        ...
        <div v-for="(file, index) in awsFileList" :key="file.Key">
            <v-img :src="`https://${awsBucketName}.s3.${awsBucketRegion}.amazonaws.com/${file.Key}`" aspect-ratio="1"
...
export default {
    ...
    data() {
        return {
            ...
            awsBucketRegion: process.env.VUE_APP_S3_BUCKET_REGION,
    ...
    methods: {
        ...
        uploadAwsS3() {
            ...
            this.s3.upload({
                ...
                if (err) {
                    console.log(err)
                    return alert('문제 있음', err.message)
                }
                alert('성공')
                this.getAwsS3Files()
            })
