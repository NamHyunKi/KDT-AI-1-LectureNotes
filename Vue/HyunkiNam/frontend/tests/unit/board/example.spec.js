...
describe('requestBoardToSpring', () => {
    test('Spring에 게시물 읽기 요청 검증', async () => {
        ...
        await expect(storeMock.actions.requestBoardToSpring({
            boardId
        })).toStrictEqual([board])
    })
})
...
describe('requestModifyBoardToSpring', () => {
    test('Spring에 게시물 수정 요청 검증', async () => {
        ...
        await expect(storeMock.actions.requestModifyBoardToSpring({ title, content, boardId })).toStrictEqual([board])
    })
})